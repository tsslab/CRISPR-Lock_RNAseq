...
# Read DE outputs
    ...
    # DM1 ASO-high treated vs DM1 ASO placebo
    path <- "/Users/seanwen/Documents/Hanifi/MARVEL/DE/Tables/"
...
for(i in 1:length(event_types)) {
    ...
    . <- data.frame("comparison"=c("DM1 ASO high dose treatment vs WT ASO placebo", "DM1 ASO high dose treatment vs WT ASO placebo (Overlap with above)"), "n.events"=c(length(df.1.small$tran_id), length(overlap)), stringsAsFactors=FALSE)
    
    path <- paste("/Users/seanwen/Documents/Hanifi/MARVEL/Pct Reversal/ASO-high/", event_types[i], "/", sep="")
