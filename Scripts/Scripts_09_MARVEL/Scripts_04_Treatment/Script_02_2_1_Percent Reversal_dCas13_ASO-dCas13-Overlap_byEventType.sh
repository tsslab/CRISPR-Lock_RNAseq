...
for(i in 1:length(event_types)) {
    ...
    . <- data.frame("comparison"=c("DM1 placebo vs WT placebo, ASO-dCas13 overlap", "DM1 dCas13 treated vs WT dCas13 placebo (Overlap with above)"), "n.events"=c(length(df.1.small$tran_id), length(overlap)), stringsAsFactors=FALSE)
    
    path <- paste("/Users/seanwen/Documents/Hanifi/MARVEL/Pct Reversal/dCas13_ASO-dCas13-Overlap/", event_types[i], "/", sep="")
    ...
    # Scatterplot: DM1 dCas13 treated vs WT dCas13 placebo
        ...
        # Save plot
        path <- paste("/Users/seanwen/Documents/Hanifi/MARVEL/Pct Reversal/dCas13_ASO-dCas13-Overlap/", event_types[i], "/", sep="")
        file <- "DM1 dCas13 treatment vs WT dCas13 placebo.pdf"
    ...
    if(event_types[i]=="SE") {
        ...
        # Scatterplot: DM1 dCas13 treated vs WT dCas13 placebo, +ve ctrls annotated
            ...
            # Save plot
            path <- paste("/Users/seanwen/Documents/Hanifi/MARVEL/Pct Reversal/dCas13_ASO-dCas13-Overlap/", event_types[i], "/", sep="")
            file <- "DM1 dCas13 treatment vs WT dCas13 placebo_Positive Controls Annotated.pdf"
            ggsave(paste(path, file, sep=""), plot, width=5, height=5)
