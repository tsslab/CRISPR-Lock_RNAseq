...
# Read all events
df <- as.data.frame(fread(paste(path, "/", "fromGTF.SE.txt", sep=""), sep="\t", header=TRUE, stringsAsFactors=FALSE))

# Remove novel events (artifacts)
#df.novel <- read.table(paste(path, "/", "fromGTF.novelJunction.SE.txt", sep=""), sep="\t", header=TRUE, stringsAsFactors=FALSE)
